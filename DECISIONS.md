...
- Owner approved using Vercel `next/image` for optimized remote posters/backdrops on 2026-06-13, replacing the temporary external image CDN approach. Image Optimization is intentionally quota-conscious: `next.config.mjs` restricts formats to WebP, limits `deviceSizes`/`imageSizes` and allowed `qualities`, sets a 24-hour minimum cache TTL, and allows only known OPhim image hosts. Poster grids remain lazy-loaded with small `sizes`; only true LCP images use priority. This still does not add `/api/image`, Sharp, filesystem image cache, or video/media proxying.
