...
- Defined the external image-cache frontend contract as `https://img.bluesia.net/i/m/<sha256>.webp?url=<encoded-normalized-upstream-image-url>` and `https://img.bluesia.net/i/d/<sha256>.webp?url=<encoded-normalized-upstream-image-url>`. When `NEXT_PUBLIC_IMAGE_CACHE_URL` is set, native `<img>`/`<picture>` poster/backdrop URLs are rewritten through that external domain; Vercel still does not fetch, validate, transform, cache, or proxy image binaries. Legacy `/image?url=` is VPS backward compatibility only and is not generated by frontend helpers.
- On 2026-06-14, `lib/images.ts` became the single source of truth for image cache URLs. It trims image sources, accepts only HTTP(S) upstream URLs, lowercases only the hostname, preserves protocol/path/query casing, removes empty fragments, hashes the normalized upstream URL with synchronous pure TypeScript SHA-256, and encodes the same normalized URL into the `url` query parameter. The variant is represented only by `/i/m/` or `/i/d/`; the hash never includes variant, width, quality, DPR, frontend host, or runtime options.
...
- If `img.bluesia.net` is unavailable and `NEXT_PUBLIC_IMAGE_CACHE_URL` is set, poster/backdrop rendering depends on the external image-cache service; unset the env var to fall back to direct upstream/CDN image URLs.
