  ...
  try {
    ...
    for (const namespace of namespaces) {
      ...
      for (const entry of entries) {
        ...
        try {
          const entryMeta = JSON.parse(metaRaw) as Partial<CacheMeta>;
          entrySize = typeof entryMeta.size === "number" ? entryMeta.size + 512 : await dirSize(dir);
        } catch {
          entrySize = await dirSize(dir);
        }
        result.push({ dir, namespace: namespace.name, size: entrySize, mtimeMs: dataInfo?.mtimeMs || 0 });
      }
    }
  } catch {
    return [];
...
export async function writeBinaryCache(namespace: string, key: string, body: Buffer, contentType: string, sourceUrl?: string): Promise<{ etag: string }> {
...
export async function writeJsonCache(namespace: string, key: string, value: unknown, sourceUrl?: string) {
  ...
  await Promise.all([
    writeFile(dataPath(namespace, key, "json"), body),
    writeFile(metaPath(namespace, key), JSON.stringify(meta))
  ]);
